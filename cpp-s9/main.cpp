...
struct PlateHasherAll {
    size_t operator()(const VehiclePlate& plate) const {
        const array<char, 3>& letters = plate.GetLetters();
        const int& l1 = letters[0];
        const int& l2 = letters[1];
        const int& l3 = letters[2];
        return static_cast<size_t>(plate.GetDigits() + plate.GetRegion() * 1000 + l1 * 100000 + l2 * 10000000 + l3 * 1000000000);
    }
};
